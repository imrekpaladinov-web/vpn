...
async def cmd_filters(m: types.Message):
    ...
    kb.button(
        text="С цифрами 🔢",
        callback_data="set_digits_on"
    )

    kb.button(
        text="Без цифр 🔤",
        callback_data="set_digits_off"
    )
...
async def perform_search(
    ...
    if user_id != ADMIN_ID:
        ...
        if (
            time.time() > stats["reset_time"]
            and stats["reset_time"] != 0
        ):
        ...
        if stats["count"] >= 10:
            ...
            await m.answer(
                "<b>❌ Лимит исчерпан. Попробуйте через 1.5 часа.</b>"
            )
    ...
    for _ in range(500):
        ...
        if mode == "5":
            ...
            name = SmartGenerator.generate_exact(
                5,
                digits
            )
    ...
    if found:
        ...
        await status.edit_text(
            "<b>❌ Ничего не найдено, попробуй еще раз!</b>"
        )
