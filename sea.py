...
async def cmd_start(m: types.Message):
    ...
    await m.answer("<b>⚔️ Приветствуем в Ghost Link Search!\n\nВыберите действие в меню ниже 👻</b>", reply_markup=get_main_menu())
...
async def cmd_filters(m: types.Message):
    ...
    kb.adjust(2)
    current = "С цифрами" if user_settings.get(m.from_user.id, {}).get("digits") else "Без цифр"
    await m.answer(f"<b>🗒️ Настройка фильтров\n\nВыберите режим поиска:\n• Текущий: {current}\n\n👻</b>", reply_markup=kb.as_markup())
...
async def handle_filter_change(c: types.CallbackQuery):
    ...
    await c.message.edit_text(f"<b>✅ Настройки обновлены: {'С цифрами 🔢' if mode else 'Без цифр 🔤'}</b>")
...
async def perform_search(m: types.Message, mode: str):
    ...
    found = None
    for _ in range(150):
        if mode == "5": name = SmartGenerator.generate_exact(5, digits)
        elif mode == "6": name = SmartGenerator.generate_exact(6, digits)
        else: name = SmartGenerator.generate_beauty_fast()
    ...
    if found:
        if user_id != ADMIN_ID:
            stats["count"] += 1
            if stats["count"] == 10: stats["reset_time"] = time.time() + 5400
        await status.edit_text(f"<b>✅ РЕЗУЛЬТАТ: @{found}\n⚡️ Время: {round(time.time() - start_time, 2)} сек.</b>")
    else:
...
async def handle_menu_buttons(m: types.Message):
    if "5 БУКВ" in m.text: await perform_search(m, "5")
    elif "6 БУКВ" in m.text: await perform_search(m, "6")
    else: await perform_search(m, "beauty")
...
async def main():
    ...
    if not await client.is_user_authorized():
        ...
        try: await client.sign_in(phone, input("Код: "))
        except SessionPasswordNeededError: await client.sign_in(password=input("Пароль (2FA): "))
    print("--- БОТ ГОТОВ ---")
